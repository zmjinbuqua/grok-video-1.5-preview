...
function fallbackGrokVideoPlan(prompt, opts) {
    return {
        prompt,
        mode: opts.mode,
...
function videoEndpoint(ctx, path, directApiKey, affinityKey) {
    if (directApiKey) {
...
export async function planGrokVideo(prompt, ctx, options = {}) {
    ...
    try {
        ...
        if (e?.code === "GENERATION_CANCELED" || options.signal?.aborted)
            throw e;
...
export async function pollVideoOnce(ctx, requestId, signal, directApiKey, affinityKey) {
    const cfg = videoConfig(ctx);
